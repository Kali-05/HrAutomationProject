...
// hr-automation-backend/utils/pdfParser.js
// hr-automation-backend/utils/pdfParser.js
const pdf = require("pdf-parse");
const { spawn } = require("child_process");
const path = require("path");
const { isATSFriendlyAndJobRelevant } = require("./atsJobFilter");
    ...
    try {
        ...
        if (!text) {
            ...
            return {
                extractedData: null,
                questions: [],
                isATSFriendlyAndJobRelevant: false,
            };
        }
        ...
        if (!extractedData) {
            return { extractedData: null, questions: [], isATSFriendlyAndJobRelevant: false };
        }

        // Check ATS-friendliness and job relevance
        const isATSFriendlyAndJobRelevantResult = isATSFriendlyAndJobRelevant(text, extractedData, jobDescription);
        console.log(`Resume ATS-friendly and job-relevant: ${isATSFriendlyAndJobRelevantResult}`);

        // Only generate questions if the resume passes the ATS and job relevance check
        let questions = [];
        if (isATSFriendlyAndJobRelevantResult) {
            // Spawn Python process to generate questions
            const questionProcess = spawn("python", [
                path.join(__dirname, "../python_scripts/generate_questions.py"),
            ]);
            ...
            console.log("Skipping question generation: Resume did not pass ATS and job relevance check");
        }

        return { extractedData, questions, isATSFriendlyAndJobRelevant: isATSFriendlyAndJobRelevantResult };
    } catch (err) {
        console.error("PDF parsing error:", err.message);
        return {
            extractedData: null,
