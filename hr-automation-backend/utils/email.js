...
// hr-automation-backend/utils/email.js
const imapSimple = require("imap-simple");
const nodemailer = require("nodemailer");
const { parseResume } = require("./pdfParser");
...
const fetchResumes = async (startDate, endDate, jobDescription) => {
    const config = {
        imap: {
            user: process.env.HR_EMAIL,
            password: process.env.HR_PASSWORD,
    ...
    // Adjust endDate to include emails received on the endDate
    const adjustedEndDate = new Date(endDate);
    adjustedEndDate.setDate(adjustedEndDate.getDate() + 1); // Increment endDate by 1 day

    const searchCriteria = [
        ["SINCE", startDate.toISOString().split("T")[0]],
        ["BEFORE", adjustedEndDate.toISOString().split("T")[0]],
    ];
    const fetchOptions = { bodies: ["HEADER"], struct: true };
    ...
    for (const message of messages) {
        ...
        if (/resume|cv/i.test(subject)) {
            console.log("Matched resume email from:", from);
            await processEmail(message, from, resumes, connection, jobDescription);
        }
    }
    console.log("Found resumes:", resumes.length);
    connection.end();
...
const processEmail = async (message, from, resumes, connection, jobDescription) => {
    if (message.attributes.struct) {
        const allStructParts = flattenStruct(message.attributes.struct);
        const attachments = allStructParts.filter(
            (part) => (part.disposition?.type === "ATTACHMENT" || part.disposition?.type === "INLINE") &&
        ...
        for (const attachment of attachments) {
            const partData = await connection.getPartData(message, attachment);
            const resumeData = await parseResume(partData, jobDescription);
            resumes.push({ email: from, resumeData, pdfData: partData });
            console.log("Resume extracted from:", from);
        }
    } else {
